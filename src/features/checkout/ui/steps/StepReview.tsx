...
export default function StepReview({
  ...
  const [waButtonVisible, setWaButtonVisible] = useState(true);
    ...
    const loadRestaurantData = async () => {
      const [hoursRes, settingsRes] = await Promise.all([
        ...
        supabase
          ...
          .select("estimated_delivery_minutes,estimated_pickup_minutes,loyalty_enabled,loyalty_points_per_eur,whatsapp_enabled,whatsapp_phone,whatsapp_message_template")
          .eq("restaurant_id", restaurantId)
          .maybeSingle(),
      ]);
      ...
      if (!settingsRes.error && settingsRes.data) {
        ...
        if (s.whatsapp_enabled && s.whatsapp_phone) {
          setWaEnabled(true);
  ...
  // 5-minute timer — hide WhatsApp button after order success
  useEffect(() => {
    if (!successOrderId || !waEnabled) return;
    setWaButtonVisible(true);
    const timer = window.setTimeout(() => setWaButtonVisible(false), 5 * 60 * 1000);
    return () => clearTimeout(timer);
  }, [successOrderId, waEnabled]);
            ...
            <a
              href={buildWhatsAppLink(
                ...
                buildOrderConfirmationMessage(
                  {
                    id: successOrderId,
                    customer_name: draft.customer.name ?? "Cliente",
                    items: [],
                    total: totalWithTip,
                    order_type: draft.orderType ?? "pickup",
                    estimated_minutes: estMins ?? undefined,
                  },
                  restaurantName ?? "",
                  waTemplate
                )
